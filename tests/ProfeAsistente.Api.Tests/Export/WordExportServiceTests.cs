...
public class WordExportServiceTests : IDisposable
{
    ...
    public WordExportServiceTests()
    {
        ...
        foreach (var name in new[] { "assessment-system-prompt.txt", "learning-guide-system-prompt.txt", "exercises-system-prompt.txt" })
        {
            ...
            if (!File.Exists(src))
                src = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "src", "ProfeAsistente.Api", "Prompts", name));
            if (File.Exists(src))
    ...
    private async Task<ProfeAsistente.Api.Models.Planificacion> CreatePlanWithClassAsync()
    {
