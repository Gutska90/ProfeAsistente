...
public class ClassroomServiceTests : IDisposable
{
    ...
    public ClassroomServiceTests()
    {
        ...
        var access = new ClassroomAccess(current);
        var clock = new SystemApplicationClock();
        var timeZones = new TimeZoneService();
        var roster = new CourseRosterService(_db, access);
        _svc = new ClassroomService(
            new TeacherDashboardService(_db, current, access, clock, timeZones),
            roster,
            new StudentSupportService(_db, current, access),
            new ClassDuaService(_db, access),
            new AttendanceService(_db, current, access, clock),
            new AssessmentService(_db, current, access, roster, clock));
    }
