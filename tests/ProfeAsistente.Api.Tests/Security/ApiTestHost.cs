...
public sealed class ApiTestHost : IAsyncDisposable
{
    ...
    public static async Task<ApiTestHost> StartAsync()
    {
        ...
        Environment.SetEnvironmentVariable("PROFEASISTENTE_JWT_KEY", "test-signing-key-32chars-minimum!!");
        Environment.SetEnvironmentVariable("PROFEASISTENTE_ADMIN_USERNAME", "admin");
        Environment.SetEnvironmentVariable("PROFEASISTENTE_ADMIN_EMAIL", "admin@test.local");
        Environment.SetEnvironmentVariable("PROFEASISTENTE_ADMIN_PASSWORD", "Admin!Pass123");
        ...
        await ProfeAsistente.Api.Data.DatabaseInitializer.InitializeAsync(app.Services);
    ...
    private static string FindApiContentRoot()
    {
        ...
        {
            var candidate = Path.Combine(dir.FullName, "src", "ProfeAsistente.Api");
            if (File.Exists(Path.Combine(candidate, "ProfeAsistente.Api.csproj")))
                return candidate;
            dir = dir.Parent;
        }

        throw new InvalidOperationException("No se encontró el content root de ProfeAsistente.Api.");
    }
