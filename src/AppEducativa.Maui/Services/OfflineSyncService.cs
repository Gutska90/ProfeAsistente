...
using AppEducativa.Shared.Offline;
...
public sealed class OfflineSyncService : IOfflineSyncService
{
    ...
    public string StatusText => PendingCount == 0
        ? "Sin cambios pendientes. La caché sirve si no hay red."
        : $"{PendingCount} cambio(s) esperando sincronizar con la API.";
    ...
    public Task<ClaseDetalleDto?> GetClaseAsync(Guid id, CancellationToken ct = default)
        => ReadThroughAsync(OfflineCacheKeys.Clase(id), () => _api.GetClaseAsync(id, ct), ct);
    ...
    private async Task EnqueueAsync(string method, string path, object body)
    {
        ...
        try
        {
            _snap.Outbox.Add(new OutboxItem
            {
                Method = method,
                Path = path,
    ...
    private void Save()
    {
        ...
        File.WriteAllText(_path, JsonSerializer.Serialize(_snap, OfflineJson.Options));
    }
}
