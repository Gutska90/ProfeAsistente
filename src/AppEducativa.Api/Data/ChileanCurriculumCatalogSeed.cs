...
public static class ChileanCurriculumCatalogSeed
{
    ...
    private static Dictionary<string, Nivel> EnsureNiveles(AppEducativaDbContext db)
    {
    ...
    private static Dictionary<string, Asignatura> EnsureAsignaturas(AppEducativaDbContext db)
    {
    ...
    private static void AddTemplateUnits(
        AppEducativaDbContext db, NivelAsignatura na, Nivel nivel, Asignatura asignatura, string displayName)
    {
    ...
    private static IEnumerable<T> TrackedAndStored<T>(AppEducativaDbContext db, DbSet<T> set) where T : class
        => db.ChangeTracker.Entries<T>().Select(e => e.Entity).Concat(set.AsNoTracking());

    internal static Guid StableId(string key)
    {
        var hash = SHA256.HashData(Encoding.UTF8.GetBytes("AppEducativa.Catalog:" + key));
