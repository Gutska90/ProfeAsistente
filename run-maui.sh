...
if ! curl -sf http://127.0.0.1:5180/health >/dev/null && ! curl -sf http://127.0.0.1:5047/health >/dev/null; then
  echo "Iniciando API..."
  (cd "$ROOT/src/AppEducativa.Api" && ASPNETCORE_ENVIRONMENT=Development dotnet run --urls http://127.0.0.1:5180) &
...
cd "$ROOT/src/AppEducativa.Maui"
dotnet build -f net8.0-maccatalyst -c Debug
open "bin/Debug/net8.0-maccatalyst/maccatalyst-arm64/AppEducativa.Maui.app"
